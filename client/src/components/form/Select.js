...
const Select = ({ onChangeHandler, value, errorMessage }) => {
  return (
    <div className="mt-5">
      <label>
        Sectors:
        <select
          ...
          className={`appearance-none outline-0 rounded w-full focus:ring p-2 bg-gray-100 mt-2 ${
            errorMessage.selector && "border border-red-600"
          }`}
          name="selector"
      ...
      <p
        className={`${
          errorMessage.selector ? "text-red-600 opacity-100 mt-1" : "opacity-0"
        }`}
      >
